...
from M2Det.layers.functions import PriorBox
from M2Det.layers.modules import MultiBoxLoss
from M2Det.data import mk_anchors
...
def adjust_learning_rate(optimizer, epoch, cfg):
    global lr
    for i in range(len(cfg.train_cfg.step_lr.ep)):
        if cfg.train_cfg.step_lr.ep[i]>=epoch:
...
def save_checkpoint(net, cfg, final=True ,epoch=10):
    if final:
        torch.save(net.state_dict(), cfg.model.weights_save + \
                'Final_M2Det_size{}_net{}.pth'.format(cfg.model.input_size, cfg.model.m2det_config.backbone))
    else:
        torch.save(net.state_dict(), cfg.model.weights_save + \
                'M2Det_size{}_net{}_epoch{}.pth'.format(cfg.model.input_size, cfg.model.m2det_config.backbone,epoch))
